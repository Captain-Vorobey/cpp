...
#include <cmath>
#include <algorithm>
#define Containers std::vector<int> v{0,0,0,8,5,11,-7,8,9,10};
std::vector<int> v1;
...
{
    Containers;
    for_each(v.begin(), v.end(), [](int& a){if(a == 0){a = 1;}});
...
void task_3(){
    Containers;
    fill_array(v.begin(),v.end(), 1, 2, 0, [](int a,int d,int number){return a + d * number;});
    //for_each(v.begin(),v.end(),[](int a){std::cout << a << " ";});
    fill_array(v2.begin(),v2.end(),1,2,0,[](int a,int d,int number){ return a * std::pow(d,number);});
...
void task_5(){
    Containers;
    int result = *find_element(v.begin() , v.end(), Even_find() );
...
void task_6(){
    ...
    std::cout << accumulate(f, f1, 0) << std::endl;
}
...
void task_7(){
    ...
    auto a = stu::accumulate(f1, make_reverse_iterator(f),0);
    std::cout << a << std::endl;
}*/

void task_8(){
    Containers;
    std::vector<int>::iterator f = find_element(v.begin(),v.end(),stu::greater);
    std::vector<int>::iterator f1 = find_element(v.begin(),v.end(),stu::less);
...
void task_10(){
    Containers;
    int result = *find_element(v.begin(), v.end(), stu::less_abs);
    std::cout << result << std::endl;
